...
def make_client(*, base_url: str, verify_ssl: bool = True,
                 ...
                 auth: Any = None) -> httpx.AsyncClient:
    ...
    - `auth` : ex. `httpx.DigestAuth(user, pwd)` pour les providers CGI/ISAPI
      (Dahua, Hikvision) qui n'ont pas de notion de token/session — l'auth
      Digest est rejouée par httpx à chaque requête.
    """
...
async def request_with_retry(client: httpx.AsyncClient, method: str, url: str,
                              *, json: Any = None, params: Any = None,
                              data: Any = None, content: Any = None,
                              headers: Any = None) -> httpx.Response:
    """Requête avec 1 retry sur 5xx transitoire (500-504 sauf 501). Log redact.

    `content`/`headers` : pour les providers envoyant un corps brut (XML ISAPI)
    plutôt que JSON/form — `data=` seul ne pose pas le bon Content-Type.
    """
    import ssl
    for attempt in (1, 2):
        try:
            r = await client.request(method, url, json=json, params=params,
                                      data=data, content=content, headers=headers)
