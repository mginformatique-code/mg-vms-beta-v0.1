...
async def ensure_path(cam: dict, *, force: bool = False) -> bool:
    """Déclare (ou met à jour) le path MediaMTX d'une caméra. Idempotent."""
    cam_id = cam["id"]
    conf = _desired_conf(cam)
    if not conf["source"].lower().startswith(("rtsp://", "rtsps://")):
        logger.warning("mediamtx.ensure_path %s : URL RTSP invalide", cam_id)
        return False
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            if not force:
                r = await client.get(f"{MEDIAMTX_API_URL}/v3/config/paths/get/{_api_path(cam_id)}")
                if r.status_code == 200:
                    existing = r.json() or {}
                    if existing.get("source") == conf["source"]:
                        return True
                    r2 = await client.patch(
                        f"{MEDIAMTX_API_URL}/v3/config/paths/patch/{_api_path(cam_id)}", json=conf)
                    return r2.status_code == 200
            else:
                await client.delete(f"{MEDIAMTX_API_URL}/v3/config/paths/delete/{_api_path(cam_id)}")
            r = await client.post(
                f"{MEDIAMTX_API_URL}/v3/config/paths/add/{_api_path(cam_id)}", json=conf)
            if r.status_code == 200:
                logger.info("mediamtx: path %s enregistré", path_name(cam_id))
                return True
            ...
            logger.warning("mediamtx: échec add path %s → HTTP %s %s",
                           cam_id, r.status_code, (r.text or "")[:200])
            return False
    except httpx.HTTPError as e:
        logger.warning("mediamtx: injoignable (%s) pour %s", type(e).__name__, cam_id)
        return False
...
async def remove_path(camera_id: str) -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            await client.delete(f"{MEDIAMTX_API_URL}/v3/config/paths/delete/{_api_path(camera_id)}")
...
async def get_path_state(camera_id: str) -> Optional[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            r = await client.get(f"{MEDIAMTX_API_URL}/v3/paths/get/{_api_path(camera_id)}")
...
async def whep_exchange(camera_id: str, offer_sdp: str) -> tuple[str, Optional[str]]:
    ...
    url = f"{MEDIAMTX_WHEP_URL}/{path_name(camera_id)}/whep"
    try:
...
async def sync_paths(cams: list[dict]) -> int:
    ...
    wanted = {c["id"] for c in cams if resolve_pipeline(c) == "mediamtx"}
    n = 0
    for cam in cams:
        if cam["id"] in wanted and await ensure_path(cam):
            n += 1
    try:
        async with httpx.AsyncClient(timeout=8) as client:
        ...
        for item in items:
            name = item.get("name", "")
            if name.startswith("camera/") and name.split("/", 1)[1] not in wanted:
                await remove_path(name.split("/", 1)[1])
