...
function IncidentRow({ item, onDetail, t }) {
  ...
  return (
    <tr onClick={() => onDetail(item)} className="border-b border-border hover:bg-secondary/50 cursor-pointer" data-testid="incident-row">
      ...
      <td className="px-3 py-2 text-xs mono">
        {isReconnect
          ? <span className="text-[#00E676]">{t("diag.reconnected")} · {fmtDuration(item.reconnect_duration_s)} · {item.reconnect_attempts || 1} {t("diag.attempts_label")}</span>
          : (item.reconnected
              ? <span className="text-[#00E676]">{t("diag.resolved")} · {fmtDuration(item.reconnect_duration_s)}</span>
              : <span className="text-[#FF3333]">{t("diag.in_progress")}</span>)}
      </td>
...
function IncidentDetail({ item, onClose, t }) {
  if (!item) return null;
  const dt = item.timestamp ? new Date(item.timestamp) : null;
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose} data-testid="incident-detail-modal">
      <div className="max-w-3xl w-full bg-card border border-border max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-4 border-b border-border flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("diag.incident_eyebrow")}</div>
        ...
        <div className="p-4 space-y-4 text-sm">
          <section>
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2">{t("diag.cause_probable")}</div>
          ...
          <section className="grid grid-cols-2 gap-3">
            <Info2 label={t("diag.field_type")} value={item.event_type} />
            <Info2 label={t("diag.field_source")} value={item.error_source} />
            <Info2 label={t("diag.field_previous_state")} value={item.previous_state} />
            <Info2 label={t("diag.field_current_state")} value={item.current_state} />
            <Info2 label={t("diag.field_uptime_before")} value={fmtDuration(item.uptime_before_incident_s)} />
            <Info2 label={t("diag.field_time_since_last_frame")} value={fmtDuration(item.time_since_last_frame_s)} />
            <Info2 label={t("diag.field_reconnect_attempts")} value={item.reconnect_attempts ?? "—"} />
            <Info2 label={t("diag.field_reconnect_duration")} value={fmtDuration(item.reconnect_duration_s)} />
            <Info2 label={t("diag.field_reconnected")} value={item.reconnected ? t("diag.value_yes") : (item.event_type === "reconnect" ? t("diag.value_yes") : t("diag.value_pending"))} />
            <Info2 label={t("diag.field_onvif_profile")} value={item.profile_name || "—"} />
          </section>
          <section className="grid grid-cols-2 gap-3">
            <Info2 label={t("diag.field_codec")} value={item.codec || "—"} />
            <Info2 label={t("diag.field_resolution")} value={item.resolution || "—"} />
            <Info2 label={t("diag.field_fps_requested")} value={item.fps_requested ?? "—"} />
            <Info2 label={t("diag.field_fps_actual")} value={item.fps_actual ?? "—"} />
            <Info2 label={t("diag.field_bitrate")} value={item.bitrate_kbps ? `${item.bitrate_kbps} kbps` : "—"} />
            <Info2 label={t("diag.field_rtsp_transport")} value={(item.rtsp_transport || "").toUpperCase() || "—"} />
          </section>
          {item.url_masked && (
            <section>
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">{t("diag.rtsp_url_header")}</div>
              <div className="text-[11px] mono bg-secondary p-2 break-all">{item.url_masked}</div>
            </section>
          )}
          {item.error_text && (
            <section>
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">{t("diag.raw_error_label")} ({item.error_text.length} {t("diag.characters")})</div>
...
function CameraSummaryCard({ camera, summary, onDownload, t }) {
  return (
    <div className="border border-border p-3" data-testid={`summary-${camera.id}`}>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={() => onDownload(camera)} className="p-1.5 hover:bg-secondary" title={t("diag.download_report_title")}>
          <Download size={14} />
        </button>
      </div>
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        <div>
          <span className="text-muted-foreground">{t("diag.disconnects_30d_label")}</span>
        ...
        <div>
          <span className="text-muted-foreground">{t("diag.avg_reconnect_label")}</span>
          <div className="mono">{summary?.avg_reconnect_s ? `${summary.avg_reconnect_s} s` : "—"}</div>
        </div>
        <div>
          <span className="text-muted-foreground">{t("diag.reconnects_label")}</span>
          <div className="mono">{summary?.reconnects_30d ?? "—"}</div>
        </div>
      </div>
      {summary?.top_causes && summary.top_causes.length > 0 && (
        <div className="mt-2 pt-2 border-t border-border">
          <div className="text-[9px] uppercase tracking-wider text-muted-foreground mb-1">{t("diag.frequent_causes_label")}</div>
...
export default function Diagnostics() {
  ...
  const load = async () => {
    ...
    } catch (e) { toast.error(t("diag.load_journal_failed")); }
  ...
  const downloadReport = async (camera) => {
    try {
      ...
      toast.success(t("diag.report_downloaded"));
    } catch (e) { toast.error(t("diag.report_generation_failed")); }
  };
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <button onClick={load} disabled={loading} data-testid="reload-diag" className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-border hover:bg-secondary">
          <RefreshCw size={13} className={loading ? "animate-spin" : ""} /> {t("diag.refresh_btn")}
        </button>
      </div>
      ...
      {cams.length > 0 && (
        <div className="mb-6">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2">{t("diag.camera_overview_title")}</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {cams.map((c) => (
              <CameraSummaryCard key={c.id} camera={c} summary={summaries[c.id]} onDownload={downloadReport} t={t} />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="border border-border p-3 mb-3 flex items-center gap-2 flex-wrap" data-testid="diag-filters">
        ...
        <select value={filter.cause} onChange={(e) => setFilter({ ...filter, cause: e.target.value })}
          ...
          <option value="">{t("diag.filter_all_causes")}</option>
        ...
        <select value={filter.event_type} onChange={(e) => setFilter({ ...filter, event_type: e.target.value })}
          ...
          <option value="">{t("diag.filter_all_events")}</option>
          <option value="disconnect">{t("diag.filter_disconnects")}</option>
          <option value="reconnect">{t("diag.reconnects_label")}</option>
        </select>
        <span className="ml-auto text-xs text-muted-foreground mono">{total} {t("diag.incidents_suffix")}</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left text-[10px] uppercase tracking-wider text-muted-foreground">
              <th className="px-3 py-2">{t("diag.th_datetime")}</th>
              <th className="px-3 py-2">{t("diag.th_camera")}</th>
              <th className="px-3 py-2">{t("diag.th_site")}</th>
              <th className="px-3 py-2">{t("diag.cause_probable")}</th>
              <th className="px-3 py-2">{t("diag.th_uptime_before")}</th>
              <th className="px-3 py-2">{t("diag.th_reconnection")}</th>
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={7} className="px-3 py-8 text-center text-muted-foreground text-sm">
                <Info size={14} className="inline mr-1" /> {t("diag.empty_journal")}
              </td></tr>
            ) : items.map((item) => (
              <IncidentRow key={item.id} item={item} onDetail={setDetail} t={t} />
            ))}
          </tbody>
        </table>
      </div>

      <IncidentDetail item={detail} onClose={() => setDetail(null)} t={t} />
...
function FrameSourceSection() {
  ...
  const modeBadge = state.mode === "cuda" ? { color: "#00E676", label: t("diag.mode_cuda_forced") }
                    : state.mode === "none" ? { color: "#666", label: t("diag.mode_cpu_forced") }
                    : state.cuvid_available ? { color: "#00E676", label: t("diag.mode_gpu_nvdec") }
                    : { color: "#FFB800", label: t("diag.mode_cpu_fallback") };
    ...
    <div className="mt-8 border border-border bg-card p-4" data-testid="frame-source-section">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div>
          <h2 className="font-head font-semibold text-lg flex items-center gap-2">
            ...
            {t("diag.frame_source_title")}
          </h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("diag.frame_source_desc_1")} <span className="mono">ffmpeg -hwaccel cuda</span> {t("diag.frame_source_desc_2")}
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      {workers.length === 0 ? (
        <div className="p-6 text-center text-muted-foreground text-sm border border-dashed border-border" data-testid="frame-source-empty">
          ...
          {t("diag.frame_source_empty")}
          {!state.cuvid_available && state.mode !== "none" && (
            <div className="mt-3 text-xs text-[#FFB800]">
              {t("diag.cuvid_warning_1")} <span className="mono">nvidia/cuda</span> {t("diag.cuvid_warning_2")}
            </div>
          )}
        </div>
      ) : (
          ...
          <table className="w-full text-xs mono" data-testid="frame-source-table">
            <thead className="border-b border-border">
              <tr className="text-left text-[10px] uppercase tracking-wider text-muted-foreground">
                ...
                <th className="px-2 py-1.5">{t("diag.field_codec")}</th>
                <th className="px-2 py-1.5">{t("diag.field_resolution")}</th>
                <th className="px-2 py-1.5">GPU</th>
                <th className="px-2 py-1.5">Restarts</th>
                <th className="px-2 py-1.5">{t("diag.th_frame_age")}</th>
                <th className="px-2 py-1.5">{t("diag.th_state")}</th>
                <th className="px-2 py-1.5">{t("diag.th_last_error")}</th>
              </tr>
            </thead>
            <tbody>
              {workers.map(([camId, w]) => {
                ...
                return (
                  <tr key={camId} className="border-b border-border/40 hover:bg-secondary/30" data-testid={`frame-source-row-${camId}`}>
                    ...
                    <td className="px-2 py-1.5">
                      {w.alive ? (
                        <span className="text-[#00E676] flex items-center gap-1"><CheckCircle2 size={12} /> {t("diag.worker_alive")}</span>
                      ) : (
                        <span className="text-[#FF3333] flex items-center gap-1"><AlertTriangle size={12} /> {t("diag.worker_dead")}</span>
                      )}
                    </td>
    ...
    <div className="mt-8 border border-border bg-card p-4" data-testid="stream-lifecycle-section">
      ...
      {selectedCam && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 text-xs">
          <div className="border border-border bg-background p-2">
            <div className="text-muted-foreground mono uppercase tracking-wider text-[10px]">{t("diag.kpi_current_status")}</div>
          ...
          <div className="border border-border bg-background p-2">
            <div className="text-muted-foreground mono uppercase tracking-wider text-[10px]">{t("diag.kpi_consecutive_probe_failures")}</div>
            <div className={`mono font-bold ${failureCount === 0 ? "text-[#00E676]" : (failureCount >= 3 ? "text-[#FF3333]" : "text-[#FFB800]")}`} data-testid="lifecycle-fail-count">
              {failureCount}/3
              {failureCount > 0 && failureCount < 3 && <span className="text-white/50 text-[10px] ml-1">{t("diag.hysteresis_label")}</span>}
            </div>
          </div>
          <div className="border border-border bg-background p-2">
            <div className="text-muted-foreground mono uppercase tracking-wider text-[10px]">{t("diag.kpi_journal_entries")}</div>
            <div className="mono font-bold" data-testid="lifecycle-entries-count">{entries.length}</div>
          </div>
          <div className="border border-border bg-background p-2">
            <div className="text-muted-foreground mono uppercase tracking-wider text-[10px]">{t("diag.kpi_last_action")}</div>
      ...
      <div className="border border-border bg-background max-h-[500px] overflow-y-auto" data-testid="lifecycle-journal">
        ...
        {!loading && entries.length === 0 && (
          <div className="p-6 text-center text-muted-foreground text-sm">
            <Info size={14} className="inline mr-1" /> {t("diag.lifecycle_empty")}
          </div>
        )}
        {entries.length > 0 && (
          <table className="w-full text-xs mono">
            <thead className="sticky top-0 bg-card border-b border-border">
              <tr className="text-left text-[10px] uppercase tracking-wider text-muted-foreground">
                ...
                <th className="px-2 py-1.5">{t("diag.th_reason")}</th>
                <th className="px-2 py-1.5">{t("diag.th_origin")}</th>
              </tr>
            </thead>
            <tbody>
              {entries.map((e, i) => {
      ...
      {Object.keys(summary).length > 1 && (
        <div className="mt-4 text-xs">
          <div className="text-muted-foreground uppercase tracking-wider text-[10px] mb-1">{t("diag.all_cameras_summary")}</div>
              ...
              return (
                <div key={camId} className="border border-border bg-background p-2 flex justify-between items-center cursor-pointer hover:border-[#00E5FF]"
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground">{s.count} {t("diag.entries_word")} · {t("diag.last_colon")} {s.last_action}</div>
                  </div>
  ...
  if (!h) {
    return (
      <div className="mt-8 border border-border bg-card p-4" data-testid="ai-health-section-loading">
        <h2 className="font-head font-semibold text-lg">{t("diag.ai_health_title")}</h2>
        <p className="text-xs text-muted-foreground mt-2">{t("diag.loading")}</p>
      </div>
    );
  }
    ...
    <div className="mt-8 border border-border bg-card p-4" data-testid="ai-health-section">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div>
          <h2 className="font-head font-semibold text-lg flex items-center gap-2">
            <Zap size={16} className="text-[#00E5FF]" />
            {t("diag.ai_health_title")}
          </h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("diag.ai_health_desc")}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <AiBadge ok={h.loop_alive} label={t("diag.badge_loop_alive")} testid="ai-badge-loop" />
        <AiBadge ok={h.yolo_loaded} label={t("diag.badge_yolo_loaded")} testid="ai-badge-yolo" />
        <AiBadge ok={h.alpr_loaded} label={t("diag.badge_alpr_loaded")} testid="ai-badge-alpr" />
        <AiBadge ok={h.torch_available} label={`PyTorch ${h.torch_version || "?"}`} testid="ai-badge-torch" />
        <AiBadge ok={h.torch_cuda_available} label={h.torch_cuda_available ? t("diag.cuda_active") : "CPU"} testid="ai-badge-cuda" />
        {h.force_cpu_env && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
        <div>
          <div className="text-muted-foreground text-[10px] uppercase tracking-wider">{t("diag.field_device_effective")}</div>
          <div className="mono font-semibold" data-testid="ai-device-effective">{h.device_effective || "—"}</div>
        </div>
        <div>
          <div className="text-muted-foreground text-[10px] uppercase tracking-wider">{t("diag.field_cycles_total")}</div>
          <div className="mono font-semibold" data-testid="ai-cycles-total">{h.cycles_total}</div>
        </div>
        <div>
          <div className="text-muted-foreground text-[10px] uppercase tracking-wider">{t("diag.field_yolo_model")}</div>
      ...
      {!h.yolo_loaded && h.yolo_error && (
        <div className="mt-4 p-3 bg-[#FF333310] border border-[#FF3333]" data-testid="ai-yolo-error">
          <div className="text-[10px] uppercase tracking-wider text-[#FF3333] font-semibold mb-1">{t("diag.yolo_error_label")} ({h.yolo_load_attempts} {t("diag.attempt_word")}{h.yolo_load_attempts > 1 ? "s" : ""})</div>
      ...
      {!h.alpr_loaded && h.alpr_error && (
        <div className="mt-2 p-3 bg-[#FF333310] border border-[#FF3333]" data-testid="ai-alpr-error">
          <div className="text-[10px] uppercase tracking-wider text-[#FF3333] font-semibold mb-1">{t("diag.alpr_error_label")} ({h.alpr_load_attempts} {t("diag.attempt_word")}{h.alpr_load_attempts > 1 ? "s" : ""})</div>
      ...
      {h.last_cycle_error && (
        <div className="mt-2 p-3 bg-[#FFB80010] border border-[#FFB800]" data-testid="ai-cycle-error">
          <div className="text-[10px] uppercase tracking-wider text-[#FFB800] font-semibold mb-1">{t("diag.last_cycle_error_label")}</div>
...
function StreamsSyncSection() {
  ...
  const repair = async () => {
    if (!confirm(t("diag.sync_confirm"))) return;
    setRepairing(true);
    try {
      const r = await api.post("/diagnostics/streams-sync/repair");
      setState(r.data);
      toast.success(t("diag.sync_success"));
    } catch (e) {
      toast.error(`${t("diag.sync_fail_prefix")} : ${e?.response?.data?.detail || e.message}`);
    } finally {
      setRepairing(false);
    }
  };
  ...
  if (!state) {
    return (
      <div className="mt-8 border border-border bg-card p-4" data-testid="streams-sync-loading">
        <h2 className="font-head font-semibold text-lg">{t("diag.sync_title")}</h2>
        <p className="text-xs text-muted-foreground mt-2">{t("diag.loading")}</p>
      </div>
    );
  }
    ...
    <div className="mt-8 border border-border bg-card p-4" data-testid="streams-sync-section">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t("diag.sync_desc_1")}
            {" "}<span className="mono">docker restart go2rtc</span> {t("diag.sync_desc_2")}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={repair} disabled={repairing || !state.go2rtc_reachable} data-testid="streams-sync-repair-btn"
            ...
            {repairing ? t("diag.resyncing") : t("diag.resync_btn")}
          </button>
        </div>
      </div>
      ...
      {!state.go2rtc_reachable && (
        <div className="p-3 bg-[#FF333310] border border-[#FF3333] mb-3" data-testid="streams-sync-unreachable">
          <div className="text-[10px] uppercase tracking-wider text-[#FF3333] font-semibold mb-1">{t("diag.unreachable_title")}</div>
          <div className="mono text-xs break-words">{state.go2rtc_error || t("diag.no_http_response")}</div>
        </div>
      )}

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs mb-4">
        <div>
          <div className="text-muted-foreground text-[10px] uppercase tracking-wider">{t("diag.kpi_db_cameras")}</div>
          <div className="mono font-semibold text-lg" data-testid="streams-sync-db-count">{state.db_cameras_count}</div>
        </div>
        <div>
          <div className="text-muted-foreground text-[10px] uppercase tracking-wider">{t("diag.kpi_go2rtc_streams")}</div>
          <div className="mono font-semibold text-lg" data-testid="streams-sync-go2rtc-count">{state.go2rtc_streams_count}</div>
        </div>
        <div>
          <div className="text-muted-foreground text-[10px] uppercase tracking-wider">{t("diag.kpi_aligned")}</div>
          <div className="mono font-semibold text-lg text-[#00E676]" data-testid="streams-sync-in-sync-count">{inSync.length}</div>
        </div>
        <div>
          <div className="text-muted-foreground text-[10px] uppercase tracking-wider">{t("diag.kpi_issues")}</div>
      ...
      {missing.length > 0 && (
        <div className="mb-3 p-3 bg-[#FF333310] border border-[#FF3333]" data-testid="streams-sync-missing">
          <div className="text-[10px] uppercase tracking-wider text-[#FF3333] font-semibold mb-2">
            {missing.length} {missing.length > 1 ? t("diag.missing_plural") : t("diag.missing_singular")}
          </div>
          {missing.map((m) => (
      ...
      {drift.length > 0 && (
        <div className="mb-3 p-3 bg-[#FFB80010] border border-[#FFB800]" data-testid="streams-sync-drift">
          <div className="text-[10px] uppercase tracking-wider text-[#FFB800] font-semibold mb-2">
            {drift.length} {drift.length > 1 ? t("diag.drift_plural") : t("diag.drift_singular")}
          </div>
          {drift.map((d) => (
      ...
      {orphans.length > 0 && (
        <div className="mb-3 p-3 bg-[#FFB80010] border border-[#FFB800]" data-testid="streams-sync-orphans">
          <div className="text-[10px] uppercase tracking-wider text-[#FFB800] font-semibold mb-2">
            {orphans.length} {orphans.length > 1 ? t("diag.orphan_plural") : t("diag.orphan_singular")}
          </div>
          {orphans.map((o) => (
      ...
      {!hasIssue && state.go2rtc_reachable && (
        <div className="p-3 bg-[#00E67610] border border-[#00E676]" data-testid="streams-sync-ok">
          <div className="flex items-center gap-2 text-[#00E676] text-xs font-semibold">
            ...
            {t("diag.sync_ok_prefix")} {inSync.length} {inSync.length > 1 ? t("diag.sync_camera_plural") : t("diag.sync_camera_singular")}
          </div>
        </div>
      )}
    </div>
