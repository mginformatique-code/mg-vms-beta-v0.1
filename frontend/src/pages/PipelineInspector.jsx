...
const STAGE_LABELS = {
  ...
  multi_anpr: "Multi-ANPR", scenarios: "pinsp.stage_scenarios", persist: "Mongo",
  ...
  return (
    <td className="px-2 py-1.5 text-center">
      ...
      <div className="text-[9px] text-muted-foreground mono">
        max {st.max_ms.toFixed(0)} · {st.calls} {t("pinsp.calls")}
...
export default function PipelineInspector() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      toast.error(t("pinsp.unavailable"));
    } finally {
      setLoading(false);
    }
  }, [t]);
    ...
    <div className="p-6 space-y-6" data-testid="pipeline-inspector-page">
      <div className="flex items-center justify-end">
        <button onClick={load} data-testid="inspector-refresh-btn"
          ...
          <RefreshCw size={13} /> {t("pinsp.refresh")}
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <SysCard icon={MemoryStick} label="RAM"
          value={sys.ram ? `${sys.ram.used_mb} / ${sys.ram.total_mb} ${t("pinsp.mb_unit")}` : "—"}
          sub={sys.ram ? `backend ${sys.ram.process_rss_mb} ${t("pinsp.mb_unit")}` : ""} />
        <SysCard icon={Zap} label="GPU"
          value={sys.gpu?.device || t("pinsp.gpu_unavailable")}
          sub={sys.gpu?.vram_allocated_mb != null
            ? `VRAM ${sys.gpu.vram_allocated_mb} / ${sys.gpu.vram_total_mb} ${t("pinsp.mb_unit")}` : "CPU only"} />
        <SysCard icon={Activity} label="Uptime" value={`${Math.round((sys.uptime_s || 0) / 60)} min`}
          sub={`${Object.keys(cameras).length} ${t("pinsp.cameras_inspected")}`} />
      </div>

      {loading && <div className="text-sm text-muted-foreground">{t("pinsp.loading")}</div>}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
                ...
                <tr className="border-b border-border/60">
                  {stageKeys.filter((s) => cam.stages[s]).map((s) => (
                    <th key={s} className="px-2 py-1.5 text-[10px] uppercase tracking-wider text-muted-foreground font-medium">
                      {STAGE_LABELS[s] === "pinsp.stage_scenarios" ? t(STAGE_LABELS[s]) : (STAGE_LABELS[s] || s)}
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                <tr>
                  {stageKeys.filter((s) => cam.stages[s]).map((s) => (
                    <StageCell key={s} st={cam.stages[s]} t={t} />
                  ))}
                </tr>
              </tbody>
            </table>
