...
// Rendu en dehors de AppProvider (voir index.js) : useApp() n'est pas
// disponible ici si le crash survient avant/pendant le montage du
// provider. On relit donc la langue directement depuis localStorage,
// en miroir de la logique par défaut d'AppContext.jsx (clé "mg_lang",
// repli "fr").
function tStandalone(key) {
  const lang = (typeof window !== "undefined" && localStorage.getItem("mg_lang")) || "fr";
      ...
      return (
        <div className="min-h-[60vh] flex items-center justify-center p-8" data-testid="error-boundary">
          <div className="max-w-lg border border-[#FF3333]/40 bg-[#FF3333]/5 p-6 space-y-3">
            <div className="text-[10px] uppercase tracking-[0.15em] text-[#FF3333]">{tStandalone("errb.unexpected_error")}</div>
            <h2 className="font-head font-black text-xl">{tStandalone("errb.section_problem")}</h2>
            <div className="text-xs text-muted-foreground">
              {tStandalone("errb.isolated_body")}
            </div>
            <details className="text-[10px] mono opacity-70">
              <summary className="cursor-pointer">{tStandalone("errb.technical_detail")}</summary>
