...
### Fixed
- **Cluster de contrôle caméra (lumière/IR/sirène/TTS/reboot) chevauchant le bouton muet du lecteur vidéo** dans l'onglet PTZ : les deux étaient ancrés au même coin bas-droite de la vue live (`LivePlayer.jsx` a son propre bouton muet/son intégré, indépendant du TTS de ce composant). Déplacé le cluster à bas-gauche, seul endroit libre dans ce contexte.

## [v3.67-button-cluster-position] — 2026-09-11 — Correctif position du cluster de boutons caméra (mosaïque)

### Fixed
- **Cluster de contrôle caméra chevauchant le bandeau de pied de tuile** (site + nom de caméra) dans la vue live en mosaïque : depuis le correctif des capacités Reolink (v3.63), davantage de boutons s'affichent réellement (lumière/IR/sirène, pas seulement TTS/reboot comme avant sur la plupart des caméras) — la rangée, plus large, revenait chevaucher le bandeau. Remonté une 2e fois (bottom-6 → bottom-10).

## [v3.66-onvif-source-token] — 2026-09-11 — Exploration auto-détection multi-objectifs (sans résultat exploitable)

### Fixed
- Extraction du champ `VideoSourceConfiguration.SourceToken` sur chaque profil ONVIF découvert, dans l'espoir de regrouper automatiquement les profils par objectif physique lors de l'ajout d'une caméra multi-objectifs. **Abandonné après test en conditions réelles** : les deux vraies caméras multi-objectifs du parc (RLC-81MA, TrackMix PoE) renvoient le même `SourceToken` pour leurs 3 profils alors qu'elles ont bien 2 capteurs physiques distincts — ce champ ONVIF standard ne fait pas cette distinction sur ce matériel. Le champ reste extrait (donnée factuelle) mais aucun regroupement automatique n'en est déduit. Prochaine piste à explorer pour ce chantier : une heuristique par motif d'URL/nom de profil, vendeur par vendeur.

## [v3.65-ptz-patrol-speed] — 2026-09-11 — Vitesse de patrouille PTZ réglable (transition entre presets)

### Added
- **Vitesse de transition entre presets réglable pendant la patrouille automatique** (curseur 10-100 %, onglet PTZ → Patrouille) — jusqu'ici la patrouille utilisait toujours la vitesse par défaut de la caméra, aucun réglage possible. Le paramètre ONVIF standard `Speed` de la commande `GotoPreset` (optionnel, PanTilt + Zoom normalisés) était déjà supporté par le standard mais jamais exploité côté MG-VMS — ajouté à `ptz_preset()` sur toute la chaîne (driver ONVIF générique, dont hérite le driver Reolink pour cette commande), à `ptz_patrol.py` (boucle de patrouille) et au clic manuel sur un preset (réutilise désormais la vitesse déjà réglée pour le joystick).
...
### Fixed
- **Root cause du tracking lent et saccadé sur TOUTES les caméras (voitures et piétons), pas seulement celles avec ANPR actif.** Signalé par l'utilisateur après plusieurs correctifs locaux sans effet perçu — signe que le vrai goulot était en amont, commun à tout le parc. Confirmé dans le code : YOLO (Ultralytics/PyTorch) et l'ANPR (`fast-alpr`, backend onnxruntime-gpu) partagent un verrou d'inférence commun depuis v3.19 (fusionné à l'époque suite à de vrais crashs CUDA — deux runtimes GPU concurrents sur le même contexte). Conséquence mesurée : une lecture ANPR (100ms à >1s **par véhicule**) bloque toute autre caméra du parc voulant lancer sa propre détection YOLO, même sans aucun rapport avec l'ANPR. Non corrigé par un simple découplage du verrou (testé puis annulé en urgence : un redémarrage serveur, effectué pour ajouter un second GPU, a remis en état une dépendance cuDNN manquante, confirmant que le risque de crash d'origine s'appliquait toujours réellement).
- **Décalage vidéo/overlay (~1s) sur les caméras 4K les plus chargées.** Le timestamp de la boîte IA était pris côté backend APRÈS l'inférence (yolo+alpr, 700ms-1.8s sur les caméras lentes) plutôt qu'à la capture de l'image, et le frontend re-timestampait encore une fois à la réception WS — la boîte affichée représentait donc une position déjà vieille de plusieurs centaines de ms, mais l'app calculait son âge depuis zéro. Le backend horodate désormais la frame à sa capture réelle ; le frontend calcule le vrai retard déjà écoulé avant d'extrapoler.
- **Gel de l'affichage IA sur les caméras à faible débit d'analyse** (ex. `portail_villeparisis`, ~0,4-1,2 img/s réel) — la fenêtre d'extrapolation (`max_prediction_ms`) figeait la position dès 800ms d'écart, systématiquement dépassé sur ces caméras. Ajustée par caméra plutôt que globalement (un réglage trop large recrée un effet de "fantôme" quand un véhicule sort du cadre ou change de trajectoire entre deux vraies détections — testé en conditions réelles, réglage affiné en conséquence).

### Added
- **Service ANPR isolé** (`backend/anpr_service.py`, conteneur `mgvms-anpr` dédié) — sépare `fast-alpr` de YOLO dans son propre processus/GPU (Quadro K620, ajoutée sur ce serveur pour ce chantier) au lieu de partager le processus/GPU de YOLO (T1000), pour lever la contention du verrou commun sans réintroduire le risque de crash CUDA de la v3.19. Sélection **par caméra** via `pipeline_config.anpr[0] = "fast-alpr-remote"` (réutilise le mécanisme existant de choix d'OCR core, aucune route nouvelle) — repli automatique et silencieux sur le chemin local si le service est injoignable, jamais de plaque perdue. Rollout progressif en cours (1 caméra de test active à la clôture de cette session, extension au reste du parc après observation).
- **Vitesse véhicule calibrée par homographie** (référence : YOLO+ByteTrack+transformation de perspective) — calibration par caméra (Camera Center → IA → "Vitesse calibrée") : 4 points cliqués sur une image de référence capturée en direct depuis le flux WebRTC (pas de dépendance à un snapshot backend, les deux routes existantes s'étant révélées peu fiables selon le codec source) + dimensions réelles du rectangle au sol → homographie calculée côté serveur (`PUT/DELETE /cameras/{id}/speed-calibration`), appliquée en direct dans la boucle de détection (avant le broadcast, aucun cycle de retard). Lissage EMA + zone morte sous 2 km/h pour qu'un véhicule à l'arrêt affiche 0 km/h franc plutôt que de "ramper" sur le bruit de détection.
- **Onglet "Tracking" dans le panneau debug app-level** (Ctrl+Shift+D) — les diagnostics par caméra (Detection FPS, Display FPS, Prédiction active, âge dernière détection) étaient déjà collectés depuis le refactor v3.36 mais sans aucun onglet pour les consulter.
- **Cache OCR par track_id** (`camera_worker.py::_stage_anpr`) — évite de relancer l'OCR complet sur un véhicule immobile déjà lu et encore valide en cache (les caches existants n'évitaient que le post-traitement après une lecture déjà effectuée, jamais l'appel OCR lui-même).
- **Sélecteur GPU0/GPU1** (barre de ressources en en-tête + page /gpu "Accélération GPU") — affiché uniquement à partir de 2 GPU détectés (comportement mono-GPU strictement inchangé), pour visualiser les métriques temps réel de chaque carte indépendamment plutôt qu'un agrégat ou le seul device 0.

### Notes techniques
- Le log "LAPI locale chargée (fast-alpr, GPU-ONNX/CUDA)" pouvait mentir : le constructeur `ALPR()` ne lève pas d'exception si `CUDAExecutionProvider` échoue à s'initialiser (onnxruntime retombe sur CPU en silence) — corrigé pour lire le provider réellement actif (`.get_providers()`) plutôt que celui demandé.
- `libcudnn.so.9` s'est avéré intermittent dans l'image (présent puis absent selon les redémarrages, cause exacte non identifiée) — sans impact sur l'objectif de cette session (lever la contention YOLO/ANPR), l'isolation par conteneur éliminant le blocage croisé indépendamment du provider onnxruntime réellement actif à un instant donné. À stabiliser séparément si l'accélération GPU de l'ANPR devient prioritaire.
- Passer un `device_id` explicite à onnxruntime pour épingler un GPU précis depuis un processus voyant les deux cartes s'est révélé peu fiable sur ce serveur — l'isolation par conteneur (GPU dédié via `NVIDIA_VISIBLE_DEVICES`) est le mécanisme retenu, pas un choix de device dans le code.

### À venir / dette connue
- Rollout du service ANPR isolé à l'ensemble du parc (1 caméra de test à la clôture de cette session) + suppression du repli verrou-partagé une fois validé sur la durée.
- Fiabiliser `libcudnn.so.9` dans l'image pour restaurer une accélération GPU ANPR fiable (actuellement intermittente).
- Calibration de vitesse à refaire avec des mesures réelles précises (le test de cette session utilisait un rectangle approximatif) avant de faire confiance aux valeurs km/h affichées.
- Audit `routers.py` (IDOR potentiel sur plusieurs endpoints détail-par-id), fenêtre pop-out second écran, checklist Phase 1 plugin anti-vol retail — reportés, non traités cette session.
...
### Added
- **Déploiement d'un modèle vision dédié (qwen2.5vl:7b) sur l'infrastructure IA existante** (VM mgai, RTX A2000 12GB, accès SSH par clé dédiée) — nécessaire car les modèles texte déjà utilisés (qwen2.5/qwen3) ne peuvent structurellement pas analyser une image. Vérifié en conditions réelles avant tout code : sur un crop réel classé « Bleu » par le classifieur CV, le modèle vision répond « Gris » à 0.95 de confiance, confirmé visuellement.
- **Correction couleur véhicule par vision** (`vehicle_color_ai.py`) : le classifieur CV existant (HSV + seuils fixes, déjà patché 2 fois) montre un biais mesuré — 6146 lectures « Bleu » contre 41 « Blanc » sur 22417, statistiquement impossible pour une flotte réelle. Tâche périodique (6h) + bouton manuel qui re-vérifie les lectures récentes et corrige si besoin, sans toucher au chemin chaud du pipeline (zéro latence ajoutée à la détection temps réel). Ignore automatiquement les images monochromes IR nocturnes (aucune couleur fiable à en tirer, réutilise le détecteur déjà en place). Interrupteur dédié `Administration → LLM`, avec statut de progression (X/Y vérifiées, Z corrigées).
- **Identification marque véhicule par vision** (`vehicle_make_ai.py`) — même architecture, mais PAS de saut sur les images IR : le logo/la calandre/la silhouette restent identifiables en niveaux de gris, contrairement à la couleur. Vocabulaire ouvert (pas d'enum fermé) avec liste de marques courantes suggérée au modèle et seuil de confiance minimum avant d'écraser une valeur existante.
- **Menu dédié « Anomalies IA »** (remplace le widget dashboard qui n'affichait que des tags bruts type « off_hours ») — le moteur de règles existant (habitudes réelles calculées par véhicule) reste le signal, Qwen le transforme en explication concrète et contextualisée. Périmètre étendu aux corrélations multi-véhicules/caméras : **convoi** (paires de plaques vues ensemble de façon répétée, filtré pour exclure les paires OCR quasi identiques et les plaques à très forte fréquence après vérification en prod — 4482 faux positifs initiaux ramenés à 19 candidats plausibles), **vague** (nombre de véhicules distincts sur une caméra très supérieur à sa médiane historique), et **plaque suspecte** (voir ci-dessous). Tâche périodique (6h) + bouton manuel + interrupteur dédié.
- **Détecteur « plaque suspecte » (confusion ANPR)** — découvert en creusant un signalement utilisateur : la plaque `AA2307JA` vue à la fois à Rieux et à Villeparisis (géographiquement incompatible). Vérifié en base : 1004 lectures sous ce texte, confiance fast-alpr 0.55-0.85 (**pas basse** — un simple relèvement de seuil n'aurait rien filtré), sur 4 caméras dont 2 sites distincts. Preuve directe obtenue via le modèle vision sur des échantillons espacés dans le temps : **4 marques réellement différentes** trouvées (BMW, Ford, Land Rover, Renault, confiance 0.95 chacune) — confirme qu'il s'agit de plusieurs véhicules réels dont les plaques dégradées/ambiguës convergent vers le même texte OCR (« attracteur »), pas un seul véhicule mal classé. Le nouveau signal croise les marques déjà vision-vérifiées par plaque et flague toute plaque associée à 2+ marques réellement différentes.
- **Fusionner 2+ identités véhicule déjà confirmées entre elles** — dans la fenêtre « Fusion & identités véhicule (IA) », section « Identités confirmées » : mode sélection (clic sur les cartes) + bouton « Fusionner (N) ». Nouvel endpoint `POST /vehicles/identities/merge`, réutilise le garde-fou de taille déjà en place.
- **Loupe circulaire** au survol de la photo principale de la fiche véhicule (zoom 3x) — pour lire la plaque ou d'autres détails sans quitter la page.
- **Nouveau signal « trajet inter-site implausible »** dans Anomalies IA — complément purement logique (aucune IA requise, coût nul) à « plaque suspecte » : compare uniquement site + horodatage, déjà stockés sur chaque lecture. Preuve trouvée sur le même cas réel : `AA2307JA` détectée à Villeparisis puis à Rieux **30 secondes plus tard** — deux sites clients réellement distincts, physiquement impossible (sévérité « high », contrairement à « plaque suspecte » qui reste probabiliste).
- **Conformité ANPR multi-pays** — audit demandé explicitement : le moteur OCR (`fast-alpr`, modèle `european-plates-mobile-vit-v2`) est déjà pan-européen, sans validation de format spécifique à la France nulle part dans le code. La configuration pays (globale + surcharge par caméra) existait déjà côté backend mais avec 2 vrais trous, corrigés : (1) Suisse et Portugal manquaient de la liste des pays sélectionnables ; (2) le badge plaque affichait « F » **en dur** dans le code sur 3 composants, quelle que soit la vraie valeur enregistrée — reflète désormais le pays réel (F/D/I/E/B/NL/GB/CH/P/USA/EU selon la lecture). Corrigé au passage : la valeur par défaut du pays global renvoyait `None` au lieu de `"fr"` documenté tant que la configuration n'avait jamais été sauvegardée manuellement (24932/24936 lectures concernées).

### Fixed
- **Fusions (validation Consensus + identités IA) invisibles partout sauf le titre d'une fiche** — root cause confirmée avec un cas réel (plaques AX217E/AX217EM) : valider une plaque canonique via le bloc Consensus multi-plugins ne réécrivait QUE le titre affiché sur la fiche détail — passages/caméras/timeline/heatmap/habitudes/anomalies/stationnement restaient tous scopés sur la seule plaque exacte demandée, et la grille principale ne consultait même pas cette validation. Nouveau résolveur unique (union des deux mécanismes de fusion existants, jusqu'ici cloisonnés l'un de l'autre) utilisé partout à la place d'une égalité exacte. Grille principale : le choix de plaque canonique respecte désormais une validation explicite au lieu du choix automatique par confiance.
- **Identité véhicule corrompue découverte et supprimée** — une identité regroupait 49 plaques manifestement sans rapport (probablement une sélection multiple accidentelle), invisible jusqu'ici car rien ne consultait cette collection pour les statistiques détaillées ; le correctif ci-dessus l'aurait sinon propagée telle quelle. Garde-fou de taille ajouté à la fois en lecture et à la création (une identité de plus de 8 plaques est désormais refusée/ignorée, jamais un OCR de plaque française plausible).
- **Stationnement affiché à tort pour un véhicule en mouvement** — la v3.25 dérivait le dwell du simple écart de temps entre 2 lectures ANPR de la même plaque : un véhicule qui repasse deux fois devant la caméra en moins de 3 minutes (demi-tour, file d'attente) déclenchait le badge sans jamais s'être réellement arrêté. Le dwell dérive désormais de l'immobilité RÉELLE du véhicule suivie par le tracker (position bbox), indépendamment de l'ANPR — se corrige immédiatement dès que le véhicule redémarre.
- **Fiche véhicule très lente à l'ouverture** — deux causes distinctes mesurées en prod : (1) `_plate_or_404` et quasiment tous les endpoints détail d'une plaque reconstruisaient un `$regex` non ancré sur une plaque déjà normalisée, empêchant tout usage d'index (22207 clés d'index parcourues contre 3 en égalité exacte, 31ms vs 1ms par appel, multiplié par ~8 endpoints à chaque ouverture) ; (2) le calcul « Consensus multi-plugins » reconstruisait un agrégat sur l'intégralité de `db.plates` à chaque appel alors que son résultat est identique quelle que soit la plaque consultée (8.5s mesurées) — mis en cache 3 minutes, par périmètre de sites. Ouverture d'une fiche : de plusieurs secondes à quasi instantané après le premier appel.
- **Onglets Caméras et Galerie retirés de la fiche véhicule** — les statistiques par caméra s'affichent désormais au survol du nom de caméra dans l'onglet Timeline (non cliquable), demande explicite pour réduire le nombre d'onglets.
- **Menu « Anomalies IA » vide alors que le moteur détectait bien des cas réels** — signalé par l'utilisateur juste après la mise en prod ci-dessus. Root cause : `vehicle_anomaly_ai_router` (route exacte `/api/vehicles/anomaly-ai`) était enregistré côté serveur APRÈS `vehicles_router`, dont la route générique `/api/vehicles/{plate}` interceptait la requête en premier (FastAPI teste les routes dans leur ordre d'enregistrement) — 404 silencieux (« vehicle_not_found », plaque='anomaly-ai') interprété par le frontend comme une liste vide. Corrigé en réordonnant l'enregistrement des routeurs. Vérifié en prod immédiatement après déploiement : le cas réel `AA2307JA` (trajet impossible Villeparisis/Rieux) et plusieurs anomalies véhicule s'affichent désormais correctement.

### Notes techniques
- Endpoint Open WebUI correct pour les appels vision : `/api/chat/completions` (natif Open WebUI), pas `/v1/chat/completions` (Ollama brut, ne fonctionne qu'en direct sur le serveur, pas via le domaine WAN qui proxifie vers Open WebUI). Après un `ollama pull` en ligne de commande (hors interface Open WebUI), un redémarrage du conteneur webui est nécessaire — sa liste de modèles est mise en cache en mémoire au démarrage.

### À venir
- Support ANPR multi-pays (Allemagne, Royaume-Uni, Suisse, Belgique, Espagne, Portugal, Italie, etc.) — pas encore audité, demande explicite.
- Détection géographique/temporelle implausible (distance entre 2 caméras vs. temps écoulé) en complément du signal « plaque suspecte » déjà livré.

## [v3.25-tracking-lissage-stationnement-natif] — 2026-09-04 — Lissage tracking temps réel, rafale mouvement, refonte visuelle Live, stationnement natif

### Added
- **Couche de lissage frontend du tracking (prédiction de mouvement) + choix du tracker par caméra.** Suite à l'audit tracking (voir artefact "Moteur de tracking MG-VMS") et à la validation d'architecture demandée avant implémentation. Bug corrigé au passage : `TrackerPool.update()` ne recevait jamais `camera`, donc `pipeline_config.tracker` ne pouvait influencer que l'étiquette affichée, jamais le tracker réellement instancié (`camera_worker.py`, `tracking.py`). Nouveau schéma imbriqué `pipeline_config.ai.{tracker,smoothing}` avec repli total sur l'ancien champ plat (zéro régression pour les 14 caméras existantes). Nouveau module frontend `frontend/src/tracking/` (`TrackState`, `MotionPredictor`, `TrackInterpolator`) — extrapole uniquement le centre de la boîte (taille constante) entre deux confirmations RÉELLES de ByteTrack, jamais au-delà de `ai.smoothing.max_prediction_ms`, ne fait ni association ni attribution d'ID (pas un 2e tracker concurrent). Désactivé par défaut (opt-in par caméra) : chemin de rendu historique inchangé tant que le lissage n'est pas activé. Boîtes en pointillé = position devinée, jamais confondues visuellement avec une vraie détection.
- **Échantillonnage en rafale sur mouvement.** Cause racine confirmée par preuve directe : 7 événements Voiture sur 8 récents avaient `track_id=null` — YOLO détecte bien le véhicule mais ByteTrack (comme BoT-SORT/OC-SORT) ne peut jamais confirmer de piste avec un seul point de détection, et à ~1-2 img/s un véhicule rapide n'a souvent qu'un point sur toute sa traversée du champ. Fix additif : réutilise exactement le signal déjà en prod de l'alerte « vive allure » (motion_pct + véhicule détecté) pour enchaîner jusqu'à 2 passages immédiats supplémentaires sur la caméra concernée, jamais si le downstream est déjà saturé.
- **Diagnostic tracking** (Detection FPS / Display FPS / Prédiction active / âge dernière détection) déplacé du panneau debug app-level vers l'onglet Debug de Suivi des performances (`PipelineCenter.jsx`) — nouveau `frontend/src/tracking/DiagnosticsRegistry.js`, chaque tuile enregistre un getter à son montage, retiré au démontage.
- **Boutons flèches pour déplacer les tuiles vidéo** (mode Organiser) — le drag&drop natif HTML5 ne fonctionne pas sur tablette/écran tactile. 4 boutons par tuile, même mécanisme de sauvegarde de disposition que le drop natif.
- **Sous-menu Journaux → Log ANPR** : tableau des véhicules les plus lus (toutes caméras, cloisonné par site) — occurrences, nombre de caméras distinctes, dernière détection, couleur/type. `GET /api/vehicles/anpr-log/top`.
- **Fenêtre unique « Fusion & identités véhicule (IA) »** (refonte demandée du bandeau de doublons toujours affiché) : bouton unique + fenêtre listant les suggestions, chaque plaque avec une vraie miniature du véhicule et le détail des attributs au survol (marque/modèle/couleur/type/lectures/dernière vue) pour permettre un vrai tri manuel. Fusionnée avec l'ancien menu « Identités véhicule » (v0.7, retiré — les deux écrivaient dans la même collection `vehicle_identities`, le détecteur de candidats heuristique de l'ancien panneau était redondant avec le dédoublonnage Qwen déjà bien plus affiné) : la 2ᵉ section de la fenêtre liste désormais les identités déjà confirmées.
- **Survol/clic sur une miniature de la timeline des passages** (fiche véhicule) : aperçu agrandi qui suit la souris au survol, ouverture plein écran au clic.
- **Stationnement natif** — nouveau suivi dwell par plaque, temps réel, indépendant des zones configurées manuellement (`SmartZonesEngine.track_plate_dwell`/`snapshot_plate_dwell`), alimenté à chaque cycle depuis les vraies lectures ANPR. Exposé via le snapshot Redis consolidé et `GET /vehicles/{plate}/parking-status` (site_scope appliqué). Nouveau bloc « En stationnement » (durée + caméra + site, live) dans l'onglet Vue de la fiche véhicule, en complément — pas en remplacement — de la statistique historique `avg_visit_duration_min` jugée peu fiable par l'utilisateur.

### Fixed
- **Pic CPU pendant la relecture d'enregistrements.** `GET /recordings/{id}/media` (transcodage HEVC→H264) n'était servi que par le conteneur backend/API, sans GPU depuis la scission en 2 conteneurs — chaque relecture tombait en repli logiciel (`libx264`), coûteux en CPU. Nouvelle commande RPC `transcode_recording` (même canal Redis que `anpr_benchmark`/`analyze_image_local`) : le conteneur API délègue au conteneur pipeline (qui a le vrai GPU), repli sur l'appel direct historique si Redis indisponible.
- **Menu Plaques toujours lent malgré la scission des conteneurs.** Root cause réelle (mesurée en prod, backend à 22% CPU) : `count_documents({})` sans filtre scanne l'intégralité des 21 547 plaques (9048ms), alors que le `find+sort+limit(50)` qui suit ne prend que 36ms. Remplacé par `estimated_document_count()` (10ms) quand aucun filtre réel n'est appliqué — même bug déjà corrigé sur `/alerts`, jamais appliqué à `/plates`.
- **Plafond de rafales simultanées** — incident réel constaté en direct (load 99,74 sur 12 cœurs, redémarrages `frame_source` en masse) : 94 déclenchements de rafale mouvement en 10 minutes sans aucun plafond global, un événement de mouvement synchronisé sur plusieurs caméras pouvant faire ajouter jusqu'à 2 passages en plus par caméra pile au moment où le système est déjà sous tension. Sémaphore global (`_MAX_CONCURRENT_BURSTS`, défaut 3), acquisition quasi non-bloquante (50ms) — une rafale ratée passe son tour plutôt que de faire la queue.
- **Plusieurs chevauchements visuels sur les tuiles de la mosaïque live**, signalés par captures d'écran successives : badge de mode WEBRTC/MJPEG/ERREUR qui chevauchait le nom de caméra puis le bouton PTZ (tous repositionnés) ; panneau diagnostic tracking qui chevauchait le bandeau de pied de tuile ; bouton mute WebRTC qui chevauchait l'horodatage ; boutons d'action caméra (`CameraControlOverlay`) qui chevauchaient le même bandeau de pied. Bandeaux passés en fond plein (`bg-black/70`) le temps d'un aller-retour, puis **revert vers le dégradé d'origine** après retour utilisateur direct sur capture (« pas ouf » en conditions réelles) — nom de caméra laissé uniquement dans le bandeau de pied (à côté du site) pour éviter le doublon avec le bandeau d'en-tête. Heure déplacée du bandeau de pied vers le bandeau d'en-tête, juste à gauche de la résolution.
- **Reconnexions ffmpeg simultanées non plafonnées** — sémaphore global (`MGVMS_AI_MAX_CONCURRENT_RECONNECTS`, défaut 4) autour du (re)démarrage ffmpeg, libéré dès la 1ère frame reçue — backstop structurel complémentaire au jitter de backoff déjà livré (v3.24).

### À venir
- **Plaque "AA2307JA" — véritable bug de reconnaissance ANPR, pas un problème de seuil de confiance** — découvert le 05/09 en creusant un signalement utilisateur (une même plaque vue à Rieux ET Villeparisis, géographiquement incompatible). Vérifié en base : 1004 lectures sous ce texte, confiance 0.55-0.85 (PAS basse — un simple relèvement de seuil ne filtrerait rien), sur 4 caméras dont 2 sites distincts, engine fast-alpr à 99%. La timeline montre des véhicules visuellement différents (couleurs, gabarits) sous la même plaque. Hypothèse la plus probable : "AA2307JA" est un texte "attracteur" pour le modèle OCR de fast-alpr — plusieurs crops ambigus/dégradés de plaques RÉELLEMENT différentes convergent vers cette même sortie, avec une confiance auto-évaluée trompeusement correcte. Piste de correctif proposée (pas encore cadrée avec l'utilisateur) : croiser la marque/couleur du véhicule (vehicle_make_ai/vehicle_color_ai, livrés le 05/09) à travers les passages d'une même plaque — une plaque dont les véhicules associés varient trop (marques different) est un signal fort de confusion OCR, à combiner avec une détection d'incohérence géographique/temporelle (distance entre 2 caméras vs. temps écoulé, vitesse requise implausible).
- **Support ANPR multi-pays** — demande explicite (05/09) : le pipeline ANPR (lecture + validation de format) doit être conforme aux formats de plaques d'autres pays européens (Allemagne, Royaume-Uni, Suisse, Belgique, Espagne, Portugal, Italie, etc.), pas seulement le format français actuellement supposé. Pas encore auditée : où le format FR est actuellement en dur dans le pipeline ANPR (validation regex, normalisation de plaque _norm_plate, réglage pays par caméra déjà existant dans anpr_config.country_override mentionné dans routes/vehicles.py — vérifier s'il couvre déjà ce besoin ou seulement l'affichage).
- **Détection d'anomalie véhicule réellement intelligente** — demande explicite (04/09) : le menu actuel (widget "Anomalies récentes") n'affiche que des tags basés sur des règles fixes (`off_hours`/`off_days`/`nocturnal_first`, comparaison brute à une fenêtre horaire calculée sur l'historique), jugé insuffisant ("pas juste tel plaque est off_hours"). Demandé : (1) un vrai **menu dédié** (pas seulement un widget dashboard), (2) une **IA avec un vrai apprentissage** produisant des informations concrètes et contextualisées plutôt que des tags bruts. Pas encore scopé — nécessite une conversation dédiée avant tout développement : quel modèle/apprentissage (fenêtre glissante par véhicule ? modèle partagé multi-véhicules ? LLM local Qwen pour la mise en récit des anomalies détectées par des règles, ou détection elle-même ML ?), quelles données d'entraînement, quel format de sortie "concret" attendu par l'utilisateur.
- **Fusionner 2 identités véhicule déjà confirmées entre elles** (dans la fenêtre "Fusion & identités véhicule (IA)", section "Identités confirmées") — demande explicite (04/09), scope clarifié après une première question de clarification : sélectionner 2+ identités déjà validées et les fusionner en une seule à 4+ plaques. Distinct du "Fusionner des fiches" existant (sélection de plaques individuelles depuis la grille principale).
- Architecture multi-tracker complète (BoT-SORT/OC-SORT sélectionnables par caméra, Norfair) — seule la couche de lissage + le switch de tracker existant sont livrés pour l'instant.
- vCenter / fédération multi-sites (liste noire partagée entre sites MG-VMS distincts, scan ANPR → alerte → propagation auto) — idée backloguée, décision d'architecture (extension site_id vs. fédération réelle) à trancher avant tout développement.
- Couverture de tests automatisés dédiée aux nouveaux modules `tracking/` et à l'échantillonnage en rafale (au-delà de la suite pytest existante, 36/39 vérifiés avant/après sans régression).

## [v3.24-fin-separation-pipeline-api] — 2026-09-03 — Fin de la séparation pipeline IA / API (2 conteneurs), correctif régression, dédoublonnage affiné

### Added
- **Scission réelle backend API / pipeline IA en 2 conteneurs (Phase 3, fin du chantier priorité n°1).** Les étapes 2a-2d (voir ci-dessous) avaient découplé tout import direct de `pipeline_v2`/`ai_engine` côté code API ; cette étape exploite ce découplage pour scinder réellement le process. Même image Docker pour les deux conteneurs — seule la variable `MGVMS_ROLE` (`api`/`pipeline`/`monolith`, défaut `monolith` = comportement historique inchangé si absente) détermine quelles tâches de fond démarrent : conteneur `backend` (rôle `api`, pas de GPU) garde les diffusions WebSocket, la supervision réseau, l'enregistrement continu, les tâches planifiées ANPR/dédup ; nouveau conteneur `pipeline` (rôle `pipeline`, GPU nvidia) garde la boucle IA, le QoS watcher, le snapshot et le canal de commandes. Vérifié en prod sur les 14 caméras : latence API ~5ms/requête pendant que le pipeline traite 120 cycles de détection/10s — preuve directe que la lenteur du menu Plaques d'origine est résolue.
- **Étapes 2a-2d de la séparation pipeline/API** (constantes d'armement/QoS extraites dans `ai_rules_settings.py` ; snapshot Redis consolidé publié toutes les 3s par le pipeline pour les lectures d'état runtime catégorie (b) ; canal de commandes Redis API→pipeline avec repli direct historique pour les écritures/mutations catégorie (c) ; RPC pour les 2 endpoints de calcul lourd à la demande — benchmark ANPR et analyse d'image — catégorie (d)). Chaque endpoint migré garde son repli sur l'ancien appel Python direct si Redis est indisponible — comportement historique garanti inchangé dans tous les cas.
- **Timeline focus (vue caméra du Mur vidéo)** : nouveau préréglage « Aujourd'hui » (en plus des fenêtres glissantes 15m/30m/1h/3h) ; bouton « Lire la vidéo » directement dans l'aperçu d'une miniature de la timeline (réutilise l'endpoint déjà utilisé par le menu Événements).

### Fixed
- **Régression découverte après la scission réelle en 2 conteneurs** : plusieurs modules hors du périmètre initial des étapes 2a-2d (audité une 2ᵉ fois par un agent indépendant, qui a trouvé 2 régressions supplémentaires) importaient encore `pipeline_v2`/`ai_engine`/`frame_source`/`gpu` en direct — dégradation silencieuse côté conteneur API (sans GPU) : caméras affichées hors ligne dans plusieurs menus, GPU « N/A » dans le header et la page `/gpu` sur TOUTES les pages, configs plugin ne se propageant plus au pipeline réel. 15 fichiers corrigés (migration vers le snapshot Redis existant ou ajout de RPC selon la catégorie). Vérifié en prod avec données réelles sur les 2 conteneurs.
- **Dédoublonnage IA — plaques quasi-identiques jamais rapprochées.** Root cause : le tri des candidats se faisait uniquement par poids (lectures cumulées), noyant les correspondances texte évidentes sous le bruit « même caméra, quelques secondes d'écart ». Tri corrigé (texte proche avant même-caméra, distance d'édition croissante, poids en dernier recours) — Qwen reste seul juge final sur chaque paire. Fiabilisé aussi le déclenchement du batch quotidien, qui pouvait ne jamais s'exécuter en pratique sur un conteneur redémarré plus souvent que son intervalle. Budget quotidien relevé 25→60.
- **Dédoublonnage — véhicules récurrents non couverts.** Un van livrant plusieurs fois sur ~1h générait une nouvelle lecture à chaque repassage, hors de la fenêtre « même passage » (120s) — jamais comparées entre elles. Fenêtre élargie à 1h, avec diversité forcée (max 3 paires par plaque par run) pour éviter qu'un seul véhicule très actif monopolise le budget quotidien de candidats.
- **Budget dédié pour les correspondances distance-1** (le signal le plus fiable) — désormais indépendant du budget partagé des correspondances plus faibles (distance 2-3), qui pouvaient laisser des paires évidentes en attente pendant des semaines.
- **Synchronisation en masse des reconnexions ffmpeg** — root cause de l'incident du 03/09 (load average 132/12 cœurs) : toutes les caméras timeoutaient dans la même fraction de seconde puis retentaient avec EXACTEMENT le même backoff fixe, ré-déclenchant la surcharge en boucle. Jitter aléatoire (0-2s) ajouté au backoff — casse la synchronisation sans dégrader la reprise d'une caméra isolée. MG-VMS reste en 4K natif (directive explicite : jamais de dégradation de résolution comme solution de contournement).

## [v3.23-debug-conteneurs-mur-video] — 2026-09-02 — Debug (conteneurs + console SSH), Journaux dédiés, Mur vidéo (pagination + disposition)

### Added
- **État des conteneurs** (Suivi des performances → Debug) : grille de cartes (mongo/redis/go2rtc/backend/frontend), point vert/rouge selon running+health, détails au survol (image/IP interne/uptime/redémarrages/ports) — via un timer systemd côté hôte qui écrit un instantané JSON, le backend n'a jamais d'accès direct à Docker (pas de socket monté).
- **Console shell hôte** (style cockpit) dans le même onglet Debug : vrai shell HÔTE (pas le conteneur), authentification entièrement déléguée à sshd/PAM avec les identifiants Linux réels de la machine, jamais ceux de MG-VMS — identifiants jamais stockés ni journalisés. WebSocket admin uniquement, JWT vérifié avant même de demander les identifiants. Frontend via xterm.js. Journal d'audit à l'ouverture/fermeture de session (jamais le contenu).
- **Centre caméras — onglet Date et heure persistant** : horloge caméra à la demande, écart avec l'heure serveur, état NTP, bouton de configuration NTP local — consultable/actionnable à tout moment sans rouvrir le formulaire d'édition. Nouveau bouton « Synchroniser maintenant » (pousse l'heure serveur en un coup, ONVIF `SetSystemDateAndTime`), distinct de la synchro NTP continue existante.
- **Mur vidéo — pagination et disposition personnalisée.** Une vue 4/9/etc. affichait toujours les N premières caméras dans l'ordre brut de l'API. Ajout d'une pagination par flèches (en boucle, pour toute taille de vue) et d'un mode « Organiser » (glisser-déposer natif, réservé technicien+) pour choisir précisément quelles caméras apparaissent à quelle position — sauvegardé en base (`GET/PUT /api/live/layout/{size}`), survit à un `upgrade.sh` contrairement à un simple `localStorage`.

### Changed
- Onglets « AI » et « Designer » retirés de Suivi des performances (vide/redondant avec Hardware/GPU/Inspector, ou déjà accessible via ses propres routes autonomes) ; titres `<h1>` redondants retirés (Inspector, Performance, Hardware) — le nom de l'onglet dans la barre suffit déjà.
- Logs récents déplacé du Debug pipeline vers un nouveau sous-menu dédié « Logs systeme » sous Journaux & Rapports (à côté de Log ANPR, Journal audit, Journal de diagnostic) — Debug ne garde que l'état des conteneurs et la console SSH.
- Avertissement proactif dans Doublons IA si le dédoublonnage LLM est désactivé (Administration → LLM) — le message d'erreur au clic affiche désormais le vrai détail renvoyé par le backend au lieu d'un message générique.

### Fixed
- **Sauvegarde `mongodump` désactivée par défaut dans `upgrade.sh`.** Les archives (~100-120 Go chacune) ne sont jamais purgées automatiquement — accumulées sur plusieurs déploiements, elles avaient rempli le disque `video-datastore` à 93% (208 Go libres sur 3 To), un vrai risque pour les enregistrements en cours. 950 Go libérés en urgence. Réactivable au coup par coup avec `--backup`.
